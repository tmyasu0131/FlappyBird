    ...
    class GameScene: SKScene, SKPhysicsContactDelegate {
        ...
        let itemCategory: UInt32 = 1 << 4       // 0...10000 //追加
        ...
        func setupItem(){
            ...
            let createItemAnimation = SKAction.run({
                ...
                // アイテムを作成
                let itemPosition = SKSpriteNode(texture: itemTexture)
                itemPosition.position = CGPoint(x: 0, y: item_y)
                
                // 衝突の時に動かないように設定する
                itemPosition.physicsBody?.isDynamic = false
                
                // スプライトに物理演算を設定する //追加
                itemPosition.physicsBody = SKPhysicsBody(rectangleOf: itemTexture.size())
                itemPosition.physicsBody?.categoryBitMask = self.itemCategory
                
                // 重力の影響を受けなくさせる
                itemPosition.physicsBody?.affectedByGravity = false
                
                item.addChild(itemPosition)
        ...
        func didBegin(_ contact: SKPhysicsContact) {
            ...
            if (contact.bodyA.categoryBitMask & itemCategory) == itemCategory || (contact.bodyB.categoryBitMask & itemCategory) == itemCategory {
                ...
                // 再生データの作成
                let mySoundAction: SKAction = SKAction.playSoundFileNamed("sound.wav", waitForCompletion: true)
                // 再生アクション.
